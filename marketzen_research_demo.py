"""
MarketZen R&D Automated Deep Research Engine with Visual Charts & Images.
Generates comprehensive 10+ page R&D market research reports for Adsy / MarketZen.
"""
import os
import asyncio
...
async def main():
    print("🚀 Initializing MarketZen Deep R&D GPT Researcher Agent...")

    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key or "ваш_реальный_ключ" in api_key or any(ord(c) > 127 for c in api_key):
        print("\n❌ Ошибка: Вставьте ваш реальный API-ключ OpenAI!")
        return

    # Set duckduckgo as free search retriever
    os.environ["RETRIEVER"] = "duckduckgo"

    researcher = GPTResearcher(
        query=RESEARCH_QUERY,
        report_type="detailed_report",  # Exhaustive multi-section deep research
        report_source="web"
    )

    print("🔎 Step 1: Conducting deep multi-section web research across 30+ sources...")
    await researcher.conduct_research()

    print("📝 Step 2: Synthesizing comprehensive R&D report with web images & visual matrices...")
    report = await researcher.write_report()

    # Append visual Mermaid diagrams and matrices
    full_report = report + "\n\n" + MERMAID_DIAGRAMS
    ...
    main_file = Path(__file__).resolve().parent / "marketzen_rd_report.md"

    timestamp_file.write_text(full_report, encoding="utf-8")
    main_file.write_text(full_report, encoding="utf-8")

    print(f"\n🎉 Глубокий отчет с диаграммами и графиками успешно сохранен!")
    print(f" 📄 Основной отчет: {main_file}")
    print(f" 📂 Архивный отчет: {timestamp_file}")
